...
1. Return experiment data
III,IV
2. Aggregate experimental data for disease
V,VI
3. Collect information for interesting gene(s)
I,VIII,IX, X
4. Integrate across genes and proteins
II,VII
...
MATCH (d:Disease {name: diseaseName})<-[:STUDIES_DISEASE]-(comp)-[:FOUND_DIFFERENTIAL]->(gene:Gene)
...
----
// Show interesting result for a given gene
MATCH (g:Gene {symbol: "PIK3CG"})<-[r:FOUND_DIFFERENTIAL]-(c:Comparison)-[:STUDIES_DISEASE]->(d:Disease)
...
----
// Show interesting results for a specific tissue type
MATCH (t:Tissue {name: "Liver"})<-[:TAKEN_FROM]-(s:Sample)-[:HAS_EXPERIMENT]->(e:Experiment)-[r:HAS_VALUE]->(g:Gene)
RETURN g.symbol AS Gene,
       t.name AS Tissue,
       r.logFC AS LogFoldChange,
       r.pValue AS PValue,
