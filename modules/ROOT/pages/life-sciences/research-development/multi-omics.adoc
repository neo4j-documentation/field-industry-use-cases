...
RETURN 
  ...
  CASE WHEN fd1.logFC > foldChangeThreshold AND fd2.logFC > foldChangeThreshold THEN 'Concordant (Overexpression)'
       WHEN fd1.logFC < foldChangeThreshold AND fd2.logFC < foldChangeThreshold THEN 'Concordant (Underexpression)'
...
WHERE 
  fd1.logFC > foldChangeThreshold AND fd2.logFC > foldChangeThreshold
RETURN 
  g.symbol AS Gene,
  g.name AS GeneName,
  fd1.logFC AS `mRNA Fold Change`,
...
RETURN 
  ...
  (rnaR.logFC + protR.logFC) / 2.0 AS Avg_FoldChange,
