...
* foldChange: Protein abundance change1
...
RETURN g.symbol AS Gene,
       ...
       CASE WHEN fd1.logFC > 0 and fd2.logFC > 0 THEN 'Concordant'
            WHEN fd1.logFC < 0 and fd2.logFC < 0 THEN 'Concordant'
...
WITH "COMP002" AS sid
MATCH (c:Comparison {sid: sid})-[fd1:FOUND_DIFFERENTIAL]->(g:Gene)-[:CODES]->(p:Protein)<-[fd2:FOUND_DIFFERENTIAL]-(c)
WHERE fd1.logFC > 0 AND fd2.logFC > 0
RETURN g.symbol AS Gene,
       g.name AS GeneName,
       fd1.logFC AS `mRNA Fold Change`,
       fd2.logFC AS `Protein Fold Change`
...
----
// Proteomics-only changes - Identify post-transcriptional regulation
WITH "Proteomics" as experimentType
MATCH (exp:Experiment {type: experimentType})-[r:HAS_VALUE]->(protein:Protein)
...
RETURN protein.name AS Protein,
       gene.symbol AS Gene,
       r.foldChange AS `mRNA Fold Change`,
       r.pValue AS `P-Value`,
...
WHERE fd.regulated = "up"
RETURN 
  DISTINCT disease.name AS Disease,
  gene.symbol AS Gene,
  gene.name AS GeneName,
  fd.logFC AS mRNA_FoldChange,
