...
WITH 
  ...
  1.5 AS foldChangeThreshold,
  0.05 AS pValueThreshold
MATCH 
    (exp:Experiment {type: experimentType})-[r:HAS_VALUE]->(protein:Protein),
    (protein)<-[:CODES]-(gene:Gene)
WHERE r.pValue < pValueThreshold AND r.foldChange > foldChangeThreshold
RETURN 
...
MATCH 
  path=(gene:Gene {symbol: geneSymbol})-[:CODES]->(p:Protein)-[i:INTERACTS_WITH*1..2]-(neighbor:Protein)<-[:CODES]-(neighbourGene:Gene)
RETURN 
