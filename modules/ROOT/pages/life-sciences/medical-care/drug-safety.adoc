...
[source, cypher, role=noheader]
----
MATCH (d:Drug)<--(:DrugCase)<-[:PRIMARY_SUSPECT_DRUG]-(c:Case)-[:HAS_REACTION]->(m:MedDraTerm)
WITH DISTINCT c, d.name AS drugName, COLLECT(DISTINCT m) AS Reactions
RETURN drugName, size(Reactions) AS numReactions, Reactions
...
```
MATCH (c:Case)-[:HAS_REACTION]->(r:MedDraTerm)
RETURN DISTINCT labels(c), count(r)
ORDER BY count(r) desc
```