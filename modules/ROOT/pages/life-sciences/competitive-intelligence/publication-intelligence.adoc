...
MATCH (pub:Publication)-[:MENTIONS]->(gene:Gene {symbol: 'TREM2'})
MATCH (pub)-[:MENTIONS]->(disease:Disease {name: "Alzheimer's Disease"})
MATCH (pub)-[:HAS_AUTHOR]->(a1:Author)-[:AFFILIATED_WITH]->(inst1:Institution)
MATCH (pub)-[:HAS_AUTHOR]->(a2:Author)-[:AFFILIATED_WITH]->(inst2:Institution)
WHERE inst1.name < inst2.name
RETURN inst1.name AS Institution1,
...
[source, cypher, role=noheader]
----
MATCH (snp:SNP)-[:ASSOCIATED_WITH]->(gene:Gene)
...
MATCH (pub:Publication)-[:MENTIONS_VARIANT]->(snp)
MATCH (pub)-[:MENTIONS]->(disease:Disease)
RETURN pub.title AS Publications,
       pub.year AS Year,
       snp.rsid AS ClinicalVariant,
       snp.variant AS Mutation,
       disease.name AS Disease
...
WITH author, collect(DISTINCT keyword.term) AS themes, count(DISTINCT pub) AS pubCount, size(collect(DISTINCT keyword.term)) > 1 AS hasMultipleThemes
WHERE size(themes) >= 2
