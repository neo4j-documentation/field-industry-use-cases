...
==== 5.1.1. Given a `Patient` what `Drug`pass:[s] were `PRESCRIBED`?
...
==== 5.1.2. Given a `Patient` what `Condition`pass:[s] were `DIAGNOSED`?
...
==== 5.1.3. Given a `Patient` and their `Condition` - what are the comorbidities of other `Patient`pass:[s] with the same `Condition`?

In this query, we want to find out all the comorbidities for the conditions that _other_ patients have. As a healthcare provider, this information could prove useful
to be able to target potential investigations or treatments.
...
==== 5.2.1. What comorbidities are associated with a particular `Condition`pass:[?]

Here we're just getting all the comorbidities associated with a particular condition. 
...
==== 5.3.1. Given a `Provider` what `Patient`pass:[s] have they seen, and how often?
...
==== 5.3.2. Given a `Provider` what `Condition`pass:[s] have they `DIAGNOSED`?
...
==== 5.3.3. Given a `Provider` what `Drug`pass:[s] have they `PRESCRIBED`?
...
==== 5.3.4. What are the providers for a given `Patient`?
...
== 6. Graph Data Science (https://neo4j.com/docs/graph-data-science/current/introduction/[GDS])
